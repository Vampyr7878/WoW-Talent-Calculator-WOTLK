...
namespace WoW_Talent_Calculator_WOTLK
{
    public partial class Talents : Form
    {
        ...
        //major glyph slots
        private RoundButton[] majors;
        //minor glyphs slots
        private RoundButton[] minors;
        //glyphs
        private Glyph[] glyphs;
        //glyph choosing dialog
        private Glyphs glyphsWindow;
        ...
        //setup glyphs interface
        private void SetGlyphs()
        {
            majors[0].Small = new Rectangle(16, 14, 49, 51);
            SetMajor(majors[0], 91, 27);
            majors[1].Small = new Rectangle(13, 17, 49, 51);
            SetMajor(majors[1], 6, 179);
            majors[2].Small = new Rectangle(14, 17, 49, 51);
            SetMajor(majors[2], 177, 179);
            minors[0].Small = new Rectangle(9, 9, 43, 45);
            SetMinor(minors[0], 15, 87);
            minors[1].Small = new Rectangle(10, 9, 43, 45);
            SetMinor(minors[1], 186, 87);
            minors[2].Small = new Rectangle(10, 9, 43, 45);
            SetMinor(minors[2], 100, 239);
            for (j = 0; j < 6; j++)
            {
                if (j < 3)
                {
                    glyphs[j] = new Glyph("Minor Glyph");
                }
                else
                {
                    glyphs[j] = new Glyph("Major Glyph");
                }
            }
        }

        //setup major glyph slots
        private void SetMajor(RoundButton major, int width, int height)
        {
            major.Size = new Size(78, 81);
            major.Location = new Point(width, height);
            major.Active = false;
            major.MouseEnter += new EventHandler(Major_MouseEnter);
            major.MouseLeave += new EventHandler(Major_MouseLeave);
            major.MouseDown += new MouseEventHandler(Major_MouseDown);
            major.MouseUp += new MouseEventHandler(Major_MouseUp);
            major.Refresh();
        }
        ...
        //load glyphs from saved file
        private void LoadGlyphs()
        {
            XmlDocument document = new XmlDocument();
            document.Load(@"Data\Glyphs.xml");
            foreach (XmlNode character in document.ChildNodes[1].ChildNodes)
            {
                if (character.ChildNodes[0].InnerText == current)
                {
                    XmlNode minor = character.ChildNodes[1].ChildNodes[0];
                    XmlNode major = character.ChildNodes[1].ChildNodes[1];
                    for (i = 0; i < 3; i++)
                    {
                        if (glyphs[i].Index >= 0)
                        {
                            glyphs[i].Name = minor.ChildNodes[glyphs[i].Index].ChildNodes[1].InnerText;
                            minors[i].Rune = @"Textures\Glyphs\" + minor.ChildNodes[glyphs[i].Index].ChildNodes[2].InnerText + ".png";
                            glyphs[i].Description = minor.ChildNodes[glyphs[i].Index].ChildNodes[3].InnerText;
                            minors[i].Active = true;
                        }
                        if (glyphs[i + 3].Index >= 0)
                        {
                            glyphs[i + 3].Name = major.ChildNodes[glyphs[i + 3].Index].ChildNodes[1].InnerText;
                            majors[i].Rune = @"Textures\Glyphs\" + major.ChildNodes[glyphs[i + 3].Index].ChildNodes[2].InnerText + ".png";
                            glyphs[i + 3].Description = major.ChildNodes[glyphs[i + 3].Index].ChildNodes[3].InnerText;
                            majors[i].Active = true;
                        }
                    }
                }
            }
        ...
        private void SaveFile()
        {
            try
            {
                if (current != null)
                {
                    ...
                    for (a = 0; a < 6; a++)
                    {
                        writer.WriteLine(glyphs[a].Index.ToString());
                    }
                    writer.Close();
                }
            }
        ...
        //event handler for major glyphs to support mouse hover
        private void Major_MouseEnter(object sender, EventArgs e)
        {
            for (i = 0; i < 3; i++)
            {
                if (sender == majors[i])
            ...
            toolTip.ToolTipTitle = glyphs[i + 3].Name;
            toolTip.Show(glyphs[i + 3].ToolTip(), (RoundButton)sender, 78, 81);
        }

        //event handler for major glyphs to support mouse hover
        private void Major_MouseLeave(object sender, EventArgs e)
        {
            toolTip.Hide((RoundButton)sender);
        }
            ...
            if (click)
            {
                if (e.Button == MouseButtons.Left)
                {
                    ...
                    try
                    {
                        ...
                        for(j = 0; j < 6; j++)
                        {
                            used[j] = glyphs[j].Name;
                        }
                        glyphsWindow = new Glyphs(current, "Major", used);
                        glyphsWindow.ShowDialog();
                        glyphs[i + 3].Name = glyphsWindow.Selected.ChildNodes[1].InnerText;
                        majors[i].Rune = @"Textures\Glyphs\" + glyphsWindow.Selected.ChildNodes[2].InnerText + ".png";
                        glyphs[i + 3].Description = glyphsWindow.Selected.ChildNodes[3].InnerText;
                        glyphs[i + 3].Index = glyphsWindow.Index;
                        majors[i].Active = true;
                        glyphsWindow.Dispose();
                    }
                    catch
                    {
                    }
                ...
                if (e.Button == MouseButtons.Right)
                {
                    glyphs[i + 3].Name = "Empty";
                    majors[i].Rune = "";
                    glyphs[i + 3].Description = "Click here to choose a Glyph to inscribe to your spellbook";
                    glyphs[i + 3].Index = -1;
                    majors[i].Active = false;
                    majors[i].Refresh();
                    Major_MouseEnter(sender, e);
                }
                click = false;
            }
        }

        //event handler for minor glyphs to support mouse hover
        private void Minor_MouseEnter(object sender, EventArgs e)
        {
            for (i = 0; i < 3; i++)
            {
                if (sender == minors[i])
            ...
            toolTip.ToolTipTitle = glyphs[i].Name;
            toolTip.Show(glyphs[i].ToolTip(), (RoundButton)sender, 62, 64);
        }

        //event handler for minor glyphs to support mouse hover
        private void Minor_MouseLeave(object sender, EventArgs e)
        {
            toolTip.Hide((RoundButton)sender);
        }

        //event handler for minor glyphs to support mouse hover
        private void Minor_MouseDown(object sender, MouseEventArgs e)
        {
            click = true;
        }

        //event handler for minor glyphs to support mouse hover
        private void Minor_MouseUp(object sender, MouseEventArgs e)
        {
            if (click)
            {
                if (e.Button == MouseButtons.Left)
                {
                    ...
                    try
                    {
                        ...
                        glyphsWindow = new Glyphs(current, "Minor", used);
                        glyphsWindow.ShowDialog();
                        glyphs[i].Name = glyphsWindow.Selected.ChildNodes[1].InnerText;
                        minors[i].Rune = @"Textures\Glyphs\" + glyphsWindow.Selected.ChildNodes[2].InnerText + ".png";
                        glyphs[i].Description = glyphsWindow.Selected.ChildNodes[3].InnerText;
                        glyphs[i].Index = glyphsWindow.Index;
                        minors[i].Active = true;
                        glyphsWindow.Dispose();
                    }
                    catch
                ...
                if (e.Button == MouseButtons.Right)
                {
                    glyphs[i].Name = "Empty";
                    minors[i].Rune = "";
                    glyphs[i].Description = "Click here to choose a Glyph to inscribe to your spellbook";
                    glyphs[i].Index = -1;
                    minors[i].Active = false;
                    minors[i].Refresh();
                    Minor_MouseEnter(sender, e);
                }
                click = false;
            }
        }
